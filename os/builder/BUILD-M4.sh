...
WORK_DIR="$OS_DIR/work/m4-firstboot"
...
IMG_NAME='ywd-hotspot-os-m4-firstboot'
...
for stage in "$HEADLESS_STAGE_SRC" "$NETWORK_STAGE_SRC" "$RUNTIME_STAGE_SRC" "$FIRSTBOOT_STAGE_SRC"; do
...
printf '==================================================\n'
printf ' YWD-Hotspot OS Image Builder - Milestone 4\n'
printf ' Secure First-Boot Wizard + Shutdown OLED\n'
printf '==================================================\n\n'
...
printf 'M4 includes:\n'
printf '  - proven M3 open setup/recovery AP + WiFi handoff\n'
printf '  - station-WiFi-gated secure first-boot appliance wizard\n'
printf '  - six-digit one-time OLED ownership code (30 minute lifetime)\n'
printf '  - HTTPS setup service on port 8443 with per-device self-signed TLS\n'
printf '  - transactional canonical config + secrets + completion marker\n'
printf '  - explicit final RF enable gate, OFF by default\n'
printf '  - OLED SHUTTING DOWN / PLEASE WAIT state on service termination\n'
printf '\nSAFETY: RF and BrandMeister remain disabled until setup explicitly enables RF.\n\n'
...
for pair in \
  ...
  "$FIRSTBOOT_STAGE_SRC|$FIRSTBOOT_STAGE_DST"; do
  src="${pair%%|*}"; dst="${pair#*|}"
  rm -rf "$dst"; mkdir -p "$dst"; cp -a "$src/." "$dst/"
done
chmod +x "$HEADLESS_STAGE_DST/01-run.sh" "$HEADLESS_STAGE_DST/files/ywd-headless-oled.py" "$HEADLESS_STAGE_DST/files/ywd-headless-provision.sh"
chmod +x "$NETWORK_STAGE_DST/01-run.sh" "$NETWORK_STAGE_DST/files/ywd-network-manager.py"
chmod +x "$RUNTIME_STAGE_DST/01-run.sh" "$FIRSTBOOT_STAGE_DST/01-run.sh"

RUNTIME_APP="$RUNTIME_STAGE_DST/files/app"
rm -rf "$RUNTIME_APP"; mkdir -p "$RUNTIME_APP"
for item in \
...
echo '[3/8] Verifying injected M4 payload...'
required_payload=(
...
for f in "${required_payload[@]}"; do
  [[ -f "$f" ]] || { echo "ERROR: injected M4 payload is incomplete: $f" >&2; exit 1; }
done
python3 -m py_compile \
  ...
  "$RUNTIME_APP/lib/setup_server.py" \
  "$RUNTIME_APP/lib/setup_admin.py"
bash -n "$RUNTIME_APP/lib/setup_entry.sh" "$RUNTIME_APP/lib/admin_dispatch.sh" "$FIRSTBOOT_STAGE_DST/01-run.sh"
printf '[OK] M4 payload staged and syntax preflight passed; app version %s\n' "$(tr -d '\r\n' < "$RUNTIME_APP/VERSION")"
...
if [[ ! -f "$DEV_KEY" ]]; then
  ...
  chmod 0600 "$DEV_KEY"; chmod 0644 "$DEV_KEY.pub"
else
...
IMG_NAME='$IMG_NAME'
PI_GEN_RELEASE='YWD-Hotspot OS M4 secure first-boot development image'
RELEASE='trixie'
ARCH='armhf'
...
rm -f "$DEPLOY_DIR/SHA256SUMS-M4"

echo '[5/8] Running builder doctor...'
bash "$OS_DIR/builder/DOCTOR.sh"

echo '[6/8] Building Raspberry Pi OS Lite + YWD-Hotspot M4...'
echo '      Runtime compilation auto-detects CPUs and is capped at four jobs.'
if [[ "$EUID" -eq 0 ]]; then (cd "$PI_GEN_DIR" && ./build.sh); else (cd "$PI_GEN_DIR" && sudo ./build.sh); fi

echo '[7/8] Generating SHA-256 checksum and verifying compressed image...'
(
  cd "$DEPLOY_DIR"; shopt -s nullglob
  files=( *"$IMG_NAME"*.img *"$IMG_NAME"*.img.xz *"$IMG_NAME"*.img.gz *"$IMG_NAME"*.zip )
  (( ${#files[@]} > 0 )) || { echo 'ERROR: pi-gen completed but no M4 deploy image was found.' >&2; exit 1; }
  sha256sum "${files[@]}" > SHA256SUMS-M4
)
M4_XZ="$(find "$DEPLOY_DIR" -maxdepth 1 -type f -name "*${IMG_NAME}*.img.xz" -print -quit)"
if [[ -n "$M4_XZ" ]]; then xz -t "$M4_XZ"; echo '[OK] XZ integrity test passed.'; fi

echo '[8/8] Build artifacts ready.'
printf '\n==================================================\n M4 BUILD COMPLETE\n==================================================\n'
find "$DEPLOY_DIR" -maxdepth 1 -type f \( -name "*${IMG_NAME}*" -o -name 'SHA256SUMS-M4' \) -printf '  %f\n' | sort
printf '\nFirst boot flow:\n'
printf '  1. No WiFi -> open YWD-Hotspot-xxxx AP -> http://10.42.0.1/\n'
printf '  2. WiFi handoff -> OLED displays six-digit M4 setup code\n'
printf '  3. Browse https://ywd-hotspot.local:8443/ (self-signed certificate warning expected)\n'
printf '  4. Finish wizard -> normal dashboard at http://ywd-hotspot.local:8080/\n'
printf '\nSSH: ssh -i %s ywd@ywd-hotspot.local\n' "$DEV_KEY"
printf 'RF remains OFF unless explicitly enabled on the final wizard page.\n'
