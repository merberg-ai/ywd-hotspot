...
(() => {
  ...
  function patchRenderer(){
    ...
    base.pluginCard=(plugin,systemEnabled,utils={})=>{
      ...
      if(origin==='uploaded'&&!plugin.installed){
        const action=`<div class="buttonrow wrap plugin-remove-source"><button class="btn danger" data-plugin-action="package-remove" data-plugin-id="${String(plugin.id).replace(/[^a-z0-9-]/g,'')}">REMOVE PACKAGE</button></div>`;
  ...
  function patchPackageActions(){
    ...
    base.handle=async(action,ctx)=>{
      ...
      if(!ok)return true;
      const done=ctx.beginBusy(ctx.button,'REMOVING…');
      try{await post('/api/plugins/package-remove',{id:ctx.id});ctx.notify(`${ctx.id} package removed`);setTimeout(()=>document.querySelector('[data-tab="plugins"]')?.click(),250);}finally{done();}
      return true;
    };
    base.__alpha182=true;
  }
  ...
  function ensureUploadUi(){
    ...
    file.onchange=async()=>{const f=file.files?.[0];file.value='';if(!f)return;if(f.size<=0||f.size>1024*1024)return notify('Plugin archive is empty or exceeds 1 MiB',true);button.disabled=true;button.textContent='VALIDATING…';try{const bytes=new Uint8Array(await f.arrayBuffer());const d=await post('/api/plugins/upload',{filename:f.name,archive_b64:bytesToB64(bytes)});const sig=d.signature||{};notify(`${d.id} uploaded · ${sig.status||'unknown'} signature · remains uninstalled`);el('pluginUploadStatus').textContent=`${d.id} is now AVAILABLE. Signature: ${sig.status||'unknown'}${sig.key_id?' · '+sig.key_id:''}.`;setTimeout(()=>document.querySelector('[data-tab="plugins"]')?.click(),250);}catch(e){notify(e.message,true);el('pluginUploadStatus').textContent=e.message;}finally{button.textContent='UPLOAD .YWDPLUGIN';button.disabled=!unlocked();}};
    const refresh=()=>{button.disabled=!unlocked();};const logout=el('logoutBtn');if(logout)new MutationObserver(refresh).observe(logout,{attributes:true,attributeFilter:['hidden']});refresh();return true;
  }

  function init(){patchRenderer();patchPackageActions();if(ensureUploadUi())return;const mo=new MutationObserver(()=>{patchRenderer();patchPackageActions();if(ensureUploadUi())mo.disconnect();});mo.observe(document.body,{childList:true,subtree:true});}
