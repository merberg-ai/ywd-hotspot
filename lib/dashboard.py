...
def talkgroup_directory(force=False):
    ...
    with TG_LOCK:
        ...
        try:
            ...
            cached_at = now; _write_disk_cache(rows, cached_at)
            ...
            if fallback:
                msg = str(exc)[:300]; TG_MEM.update(rows=fallback, cached_at=fallback_at, stale=True, error=msg)
...
def search_talkgroups(query="", ids=None, limit=40, force=False):
    ...
    ids = ids or []; idset = {_tg_id(x) for x in ids}; idset.discard(None)
    q = " ".join(str(query or "").lower().split())[:80]; terms = q.split()
    def match(row):
        tid = int(row["id"])
    ...
    if not idset:
        def score(row):
            ...
            return (0 if sid == q else 1 if name == q else 2 if sid.startswith(q) else 3 if name.startswith(q) else 4,
                    len(name), int(row["id"]))
        found.sort(key=score)
    else:
        found.sort(key=lambda r: int(r["id"]))
    found = found[:max(1, min(int(limit), 100))]
    return {"ok": True, "results": found, "directory_count": len(rows), "cached_at": cached_at,
...
class H(core.H):
    def do_GET(self):
        ...
        if path in ("/", "/index.html") and not setup_complete():
        ...
        if path == "/api/talkgroups/search":
            ...
            if force and not core.authenticated(self.headers):
                self.send_json({"error": "Unlock control mode before forcing a BrandMeister directory refresh"}, 401); return
...
def main():
    c = core.canonical_cfg(); w = c.get("web", {})
    bind = w.get("bind", "0.0.0.0"); port = int(w.get("port", 8080))
