...
PERSIST_DIR = Path("/var/lib/ywd-hotspot-os")
AP_SECRET = PERSIST_DIR / "setup-ap-password"
...
def scan_networks():
    ...
    for line in out.splitlines():
        ...
        try:
            ...
            sig = 0
        old = best.get(ssid)
        if old is None or sig > old["signal"]:
            best[ssid] = {"ssid": ssid, "signal": sig, "security": security or "OPEN"}
...
def load_builder_credentials():
    if not PROVISION.is_file():
        return None
    # provision.env is generated by our root-owned builder using shell-safe %q.
    # Source it with a static shell snippet; no file content is interpolated.
    p = run(
        [
            "/bin/bash",
            "-c",
...
def ensure_ap_identity():
    mac = "0000"
    try:
        mac = Path(f"/sys/class/net/{WLAN}/address").read_text().strip().replace(":", "")[-4:].upper()
    except Exception:
        pass
    ssid = f"YWD-Hotspot-{mac}"
    PERSIST_DIR.mkdir(parents=True, exist_ok=True)
    if AP_SECRET.exists():
        password = AP_SECRET.read_text().strip()
    else:
        alphabet = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789"
        password = "".join(secrets.choice(alphabet) for _ in range(10))
        tmp = AP_SECRET.with_suffix(".tmp")
        tmp.write_text(password + "\n")
        os.chmod(tmp, 0o600)
        os.replace(tmp, AP_SECRET)
...
class SetupHandler(BaseHTTPRequestHandler):
    server_version = "YWD-Hotspot-Setup/0.1"
...
class NetworkManager:
    def __init__(self):
        ...
        self.networks = []
        self.ap_ssid, self.ap_password = ensure_ap_identity()
        self.httpd = None
    ...
    def write_state(self, **extra):
        state = {
            ...
            "ap_password": self.ap_password if self.mode in ("setup_ap", "recovery_ap") else "",
    ...
    def start_ap(self, recovery, reason):
        with self.lock:
            self.connecting = False
        self.mode = "recovery_ap" if recovery else "setup_ap"
        self.reason = reason
        # Scan before switching to AP; many single-radio adapters cannot scan well
        # while actively hosting an AP.
        ...
        if not add or add.returncode != 0:
            self.reason = "Could not create setup AP profile"
            self.write_state()
            return False
        mod = run(
            [
                ...
                "802-11-wireless-security.psk", self.ap_password,
                "ipv4.method", "shared",
        ...
        if not mod or mod.returncode != 0:
            self.reason = "Could not configure setup AP profile"
            self.write_state()
            return False
        up = run(["nmcli", "--wait", "30", "connection", "up", AP_PROFILE], timeout=38)
        if not up or up.returncode != 0:
            self.reason = "Could not start setup AP"
            self.write_state()
            return False
        self.write_state(ip=AP_IP)
        self.start_http()
        return True
...
<div class="card">
  ...
  <p class="small">The AP will disappear while the hotspot tries the new network. If it fails, this recovery AP returns automatically.</p>
</div>
"""
